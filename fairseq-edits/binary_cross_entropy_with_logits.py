...
class BinaryCrossEntropyWithLogitsCriterionConfig(BinaryCrossEntropyCriterionConfig):
    ...
    #for the adjusted loss function
    kg_lambda: float = field(
        default = 0.0,
        metadata={
            "help": "value for making the penalty count"
        }
    )
    kg_edges_file: Optional[str] = field(
        default=None,
        metadata={
            "help": "connects the file with the child-parent connections between the labels"
        }
    )
    kg_label_def_file: Optional[str] = field(
        default=None,
        metadata={
            "help": "connects the file with the label names"
        }
    )
...
class BinaryCrossEntropyWithLogitsCriterion(BinaryCrossEntropyCriterion):
    ...
    def __init__(self, cfg: BinaryCrossEntropyWithLogitsCriterionConfig, task: Task):
        ...
        #for the new loss function

        self.kg_lambda = cfg.kg_lambda

        if cfg.kg_edges_file is None or cfg.kg_label_def_file is None:
            self.kg_edges = []
        else:
            with open(cfg.kg_label_def_file, 'r') as l:
            ...
            with open(cfg.kg_edges_file, 'r') as e:
                    label_edges = csv.reader(e)
                    next(label_edges)
                    ...
                    for row in label_edges:
                        ...
                        kg_edges.append((child_index, parent_index))
                    

            self.kg_edges = kg_edges

            #debugging
            #print("KG edges:", len(self.kg_edges))
            #print(self.kg_edges[:5])
    ...
    def compute_loss(
        ...
        #print("BCE loss:", bce_loss.detach().item())

        kg_loss = torch.tensor(0.0, device=logits.device)

        #if we're doing the special loss
        #always compute violation rate for diagnostics, regardless of lambda
        if len(self.kg_edges) > 0:
            probabilities = torch.sigmoid(logits)
            violation_count = 0

            for (child_idx, parent_idx) in self.kg_edges:
                p_child = probabilities[:, child_idx]
                p_parent = probabilities[:, parent_idx]
                violation_count += (p_child > p_parent).sum().item()

            kg_violation_rate = violation_count / (len(self.kg_edges) * logits.shape[0])
        else:
            kg_violation_rate = 0.0

        # only compute kg penalty loss if lambda > 0
        if self.kg_lambda > 0 and len(self.kg_edges) > 0:
            for (child_idx, parent_idx) in self.kg_edges:
                p_child = probabilities[:, child_idx]
                p_parent = probabilities[:, parent_idx]
                kg_loss += torch.relu(p_child - p_parent).sum()

            kg_loss = kg_loss / len(self.kg_edges)

        loss = bce_loss + self.kg_lambda * kg_loss

            #debugging
            #print("KG penalty:", penalty.item())

        return loss, [bce_loss.detach(), kg_loss.detach(), (self.kg_lambda * kg_loss).detach(), torch.tensor(kg_violation_rate)]


    def forward(self, model, sample, reduce=True):
        loss, sample_size, logging_output = super().forward(model, sample, reduce=reduce)

        # super().forward() called compute_loss which returned [bce_loss, kg_loss, kg_weighted]
        # those ended up as loss_0, loss_1, loss_2 in logging_output
        # pull them out and re-log under readable names

        if "loss_0" in logging_output:
            logging_output["bce_loss"] = logging_output["loss_0"]
            logging_output["kg_loss"] = logging_output["loss_1"]
            logging_output["kg_weighted"] = logging_output["loss_2"]
            logging_output["kg_violation_rate"] = logging_output["loss_3"]
    ...
    def get_logging_output(self, logging_output, logits, target, sample, net_output=None):
        ...
        if self.report_auc:
            probs = torch.sigmoid(logits)
            BinaryCrossEntropyWithLogitsCriterion._val_y_true.append(
                target.cpu().numpy()
            )
            BinaryCrossEntropyWithLogitsCriterion._val_y_score.append(
    ...
    def reduce_metrics(logging_outputs, prefix: str = None) -> None:
        ...
        bce_sum = utils.item(sum(log.get("bce_loss", 0) for log in logging_outputs))
        kg_sum = utils.item(sum(log.get("kg_loss", 0) for log in logging_outputs))
        kg_weighted_sum = utils.item(sum(log.get("kg_weighted", 0) for log in logging_outputs))
        kg_violation_sum = utils.item(sum(log.get("kg_violation_rate", 0) for log in logging_outputs))
        ...
        metrics.log_scalar(
            f"{prefix}kg_loss",
            kg_sum / (sample_size or 1),
            sample_size,
            round=6,
        )

        metrics.log_scalar(
            f"{prefix}kg_weighted",
            kg_weighted_sum / (sample_size or 1),
            sample_size,
            round=6,
        )

        metrics.log_scalar(
            f"{prefix}kg_violation_rate",
            kg_violation_sum / len(logging_outputs),  # average rate, not divided by sample_size
            sample_size,
            round=6,
        )
        ...
        if len(BinaryCrossEntropyWithLogitsCriterion._val_y_true) > 0:
            y_true = np.concatenate(BinaryCrossEntropyWithLogitsCriterion._val_y_true)
            y_score = np.concatenate(BinaryCrossEntropyWithLogitsCriterion._val_y_score)
            metrics.log_custom(meters.AUCMeter, f"_{prefix}auc", y_score, y_true, np.array([]))
            # clear for next epoch
            BinaryCrossEntropyWithLogitsCriterion._val_y_true = []
            BinaryCrossEntropyWithLogitsCriterion._val_y_score = []
        ...
        metrics.log_derived(
            f"{prefix}auprc",
            lambda meters: safe_round(meters[f"_{prefix}auc"].auprc, 5)
            if f"_{prefix}auc" in meters else float("nan")
        )
